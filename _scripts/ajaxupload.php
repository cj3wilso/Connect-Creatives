...
<?php
	if($imgUploaded){
		...
		if (mysql_fetch_array( $idtest ) == ""){
			$prorecord = mysql_query("INSERT INTO cc_profile (user_id, profile_pic) VALUES ('$id','$upload_image') ;");
		}else{
			$prorecord = mysql_query("update cc_profile set profile_pic='$upload_image' where user_id = '$id';");
		}
		echo '<img src="_images/success.gif" width="16" height="16" border="0" style="marin-bottom: -4px;" /> Success!<br /><img src="'.$upload_image.'" border="0" />';
	}else{
		echo '<img src="_images/error.gif" width="16" height="16px" border="0" style="marin-bottom: -3px;" /> Error(s) Found: ';
